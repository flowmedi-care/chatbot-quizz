...
module.exports = async (req, res) => {
  ...
  const cadernoId = Number(req.query?.cadernoId || (req.body && req.body.cadernoId));
  if (!Number.isFinite(cadernoId) || cadernoId <= 0) {
    return res.status(400).json({ error: "cadernoId invalido." });
  }
  if (!groupJid) {
  ...
  try {
    ...
    const { data: caderno, error: cErr } = await supabase
      .from("cadernos")
      .select("id, target_group_jid, delivery_mode")
      .eq("id", cadernoId)
      .maybeSingle();
    ...
    console.error(e);
    return res.status(500).json({ error: e.message || "Erro no engajamento do caderno" });
  }
};
